// Package certkit holds the AT certification §5 walkthrough — fixtures, the
// thirteen checklist scenarios, and the artefact writers (SAF-T, PDF, checklist)
// — factored out of cmd/demo so more than one binary can drive the same set of
// documents. Scenarios issue through an Issuer seam (issuer.go): cmd/demo wires
// a DomainIssuer (straight to the domain layer), cmd/appsmoke wires an issuer
// backed by the internal/app service layer.
package certkit
