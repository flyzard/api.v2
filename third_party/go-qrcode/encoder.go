...
//
// Forked for github.com/flyzard/invoicing.v2: the encode() method is modified
// to always produce a SINGLE ISO/IEC 18004 Byte-mode segment regardless of
// content, satisfying the AT (Autoridade Tributária) requirement for QR codes
// on Portuguese tax documents (AT compliance C3). All other logic is unchanged.
...
// encode data as a single Byte-mode segment and return the encoded data.
//
// Fork note: the upstream encoder auto-selects mixed alphanumeric+byte
// segments to minimise symbol size. This fork forces a SINGLE dataModeByte
// segment unconditionally, as required by AT (Autoridade Tributária) for
// Portuguese tax document QR codes (compliance item C3). ISO/IEC 18004 §7.4.5
// Byte mode encodes each character as one 8-bit codeword; the mode indicator
// is 0100 (binary). The character count uses numByteCharCountBits bits.
//
...
func (d *dataEncoder) encode(data []byte) (*bitset.Bitset, error) {
	...
	// C3 fork: always use a single Byte-mode segment. Skip the mode
	// classifier and optimiser entirely.
	d.optimised = []segment{{dataMode: dataModeByte, data: d.data}}
